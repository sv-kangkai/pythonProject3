...
def webdriver_example():
    ...
    if driver['DRIVER'] == 'FIREFOX':
        browser = webdriver.Firefox(executable_path=r'{driver_path}'.format(driver_path=driver['DRIVER_PATH']),
                                    proxy=smartproxy())
    elif driver['DRIVER'] == 'CHROME':
        browser = webdriver.Chrome(executable_path=r'{driver_path}'.format(driver_path=driver['DRIVER_PATH']),
